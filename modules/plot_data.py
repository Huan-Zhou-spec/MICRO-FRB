...
def plot_scatter_with_choice(repeater_file, non_repeater_file, x_axis='dm'):
    ...
    if len(x_rep) > 0:
        sc1 = ax.scatter(x_rep, y_rep, c=snr_rep, cmap=cmap,
                         ...
                         label=rf'Repeaters ($N_{{\rm R}}$={len(x_rep)})')
    ...
    if len(x_non) > 0:
        sc2 = ax.scatter(x_non, y_non, c=snr_non, cmap=cmap,
                         ...
                         edgecolors='black', linewidth=0.5,
                         label=rf'Non-repeaters ($N_{{\rm NR}}$={len(x_non)})')
...
def plot_qq_matrix(samples_list, labels=None,
                   highlight_pairs=None,
                   highlight_colors=None, highlight_linewidth=3, highlight_alpha=0.8,
                   dull_colors=None, dull_linewidth=1.5, dull_alpha=0.5,
                   figsize=(10, 8),
                   xlabel='First spectrum', ylabel='Second spectrum',
                   show_legend=True, legend_fontsize=16,
                   label_fontsize=16, tick_fontsize=16,
                   same_scale=True, normalize=True,
                   pairs_subset=None):
    """
    绘制 Q-Q 矩阵，高亮对使用实线和不同颜色，非高亮对使用虚线和另一套颜色。
    ...
    参数:
        ...
        labels : list of str, optional
            峰的标签，默认 'Peak i'。
        highlight_pairs : list of (i,j) or boolean matrix
            需要高亮显示的峰对。
        highlight_colors : list of colors, optional
            高亮对的颜色列表，长度需与高亮对数量一致。若未提供则自动从 'tab10' 循环。
        highlight_linewidth : float, default 3
            高亮对线宽。
        highlight_alpha : float, default 0.8
            高亮对透明度。
        dull_colors : list of colors, optional
            非高亮对的颜色列表，长度需与所有非高亮对数量一致。若未提供则自动从另一套 colormap 循环（如 'Set2'）。
        dull_linewidth : float, default 1.5
            非高亮对线宽。
        dull_alpha : float, default 0.5
            非高亮对透明度。
        ...
        normalize : bool, default True
            是否将每个样本归一化到 [0,1]。
        pairs_subset : list of (i,j), optional
            只绘制指定的峰对（全子集）。
    """
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    if normalize:
        ...
        for s in samples_list:
            ...
            if s_max - s_min == 0:
                norm_samples.append(np.full_like(s, 0.5))
            else:
                norm_samples.append((s - s_min) / (s_max - s_min))
    else:
        norm_samples = samples_list

    # 处理高亮集合
    if highlight_pairs is not None:
        if isinstance(highlight_pairs, np.ndarray) and highlight_pairs.shape == (n, n):
            highlight_set = {(i, j) for i in range(n) for j in range(i+1, n) if highlight_pairs[i, j]}
        elif isinstance(highlight_pairs, list):
            highlight_set = set(highlight_pairs)
        else:
            raise ValueError("highlight_pairs must be a list of (i,j) or boolean matrix")
    else:
        highlight_set = set()
    ...
    # 区分高亮与非高亮
    highlight_pairs_list = [p for p in pairs_to_plot if p in highlight_set]
    dull_pairs_list = [p for p in pairs_to_plot if p not in highlight_set]

    # 颜色分配
    if highlight_colors is None:
        # 高亮使用鲜艳 colormap
        highlight_colors = [plt.cm.tab10(i % 10) for i in range(len(highlight_pairs_list))]
    if dull_colors is None:
        # 非高亮使用柔和/不同 colormap，例如 Set2 或 tab20
        dull_colors = [plt.cm.Set2(i % 8) for i in range(len(dull_pairs_list))]

    fig, ax = plt.subplots(figsize=figsize)

    all_data = np.concatenate(norm_samples)
    ...
    # 绘制非高亮对（虚线）
    handle_map = {}
    for idx, (i, j) in enumerate(dull_pairs_list):
        s_i = np.sort(norm_samples[i])
        s_j = np.sort(norm_samples[j])
        color = dull_colors[idx % len(dull_colors)]
        line, = ax.plot(s_i, s_j,
                        color=color, linestyle='--',
                        linewidth=dull_linewidth, alpha=dull_alpha,
                        label=None)   # 不加入图例
        handle_map[(i,j)] = line

    # 绘制高亮对（实线，不同颜色，加入图例）
    highlight_handles = []
    for idx, (i, j) in enumerate(highlight_pairs_list):
        s_i = np.sort(norm_samples[i])
        s_j = np.sort(norm_samples[j])
        color = highlight_colors[idx % len(highlight_colors)]
        line, = ax.plot(s_i, s_j,
                        color=color, linestyle='-',
                        linewidth=highlight_linewidth, alpha=highlight_alpha,
                        label=f'{labels[i]} vs {labels[j]}')
        highlight_handles.append(line)

    # 图例只显示高亮对
    if show_legend and highlight_handles:
