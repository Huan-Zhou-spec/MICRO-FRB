...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 29 21:38:34 2026
...
def plot_dynamic_spectrum(data_dict, frb_name, peak_indices=None, time_unit='ms',
    ...
    if scale_region_indices is not None and len(scale_region_indices) >= 2:
        ...
        if len(indices) < 2:
            ...
            scaled_h = orig_h / scaling_factor + (1-1/scaling_factor)*B2
...
def get_fullband_total_noise_mean(frb_name, peak1_index, peak2_index,window_factor=1,
                                   signal_factor=2.5,
                                   chime_file='FRB_data/CHIME_cat2_frb/chimefrbcat2.npy',
                                   data_dir='FRB_data/canfar_downloads/'):
    """
    提取全频带总光变曲线 total = sum(dynamic, axis=0) 的平均噪声。
    信号区域边界基于 extract_with_fwhm 中计算的总光变曲线（仅观测频带）确定。
    """
    # 1. 读取动态谱并处理（与 extract_with_fwhm 完全一致）
    ...
    dynamic = proc_data['dynamic_spectrum']          # 形状 (freq, time)
    T_len = dynamic.shape[1]

    # 2. 调用一次 extract_with_fwhm 以获取 fwhm1, fwhm2（信号区域边界计算需要）
    #   注意：此调用会重复读取和处理数据，若性能敏感可重构，但简单场景下可接受
    res = extract_with_fwhm(frb_name, peak1_index, peak2_index,
                            window_factor, signal_factor,
                            chime_file, data_dir)
    fwhm1 = res['fwhm1']
    fwhm2 = res['fwhm2']

    # 3. 计算信号区域边界（与 extract_with_fwhm 内部一致）
    ...
    # 4. 全频带总光变曲线（所有频率通道求和）
    total_full = np.sum(dynamic, axis=0)   # 长度为 T_len

    # 5. 提取噪声区域：信号区域之外的左右两段
    left_noise = total_full[:signal_start] if signal_start > 0 else np.array([])
    right_noise = total_full[signal_end:] if signal_end < T_len else np.array([])

    # 6. 合并噪声段并计算平均值
    if left_noise.size == 0 and right_noise.size == 0:
        return np.nan
    noise_all = np.concatenate([left_noise, right_noise]) if left_noise.size and right_noise.size else (left_noise if left_noise.size else right_noise)
    return np.mean(noise_all)


# 您已有的参数
frb_name = 'FRB20211115A'
peak_indices = [78, 81, 85]           # 1,2,3 号峰
green_peak_indices = [78, 85]          # peak1 和 peak3 用绿色

# 缩放平移参数
scale_region = [80, 81, 82, 83]        # peak2 附近区域
Rf = 1.77                              # 缩放因子（压缩时间）
dp_index = round(6.86 / 0.98)
...
B2 = get_fullband_total_noise_mean(frb_name, 78, 85, window_factor=1,
                                   signal_factor=10)
